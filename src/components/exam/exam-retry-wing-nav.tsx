...
export function ExamRetryWingNav({
  ...
  if (totalRetryCount <= 0) return null;

  const canGoPrev = retryCount > 0;
  const canGoNext = retryCount < totalRetryCount;

  const wingBase =
    "fixed top-1/2 z-30 flex size-11 -translate-y-1/2 items-center justify-center rounded-full bg-white text-blue-950 shadow-lg ring-1 ring-zinc-200 transition-opacity hover:bg-zinc-50 disabled:pointer-events-none disabled:opacity-30";
